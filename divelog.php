...
function plugin_init(){
	...
	update_option('inm_name','yes');
	
	update_option('date','yes');
	update_option('location','yes');
	...
	update_option('bottom_time_icon_path','bottom_time.gif');	
...
function post_data($id) {
	...
	if (isset($bottom_time) && !empty($btom_time)) {
...
<?php 
$scuba_club_link = get_option('scuba_club');
if  ($scuba_club == 'yes') { 
		...
		<table width="100%" height="50px" align="center"  border="0">
			...
			if (get_option('bottom_time_icon') == 'yes'){ 
				if (get_post_meta($post->ID, bottom_time, true) != ''){ ?>			
					<td width="50px"  background="<?php echo $wp.ICONS_FILEPATH  . get_option('bottom_time_icon_path')?>" style="background-repeat:no-repeat; padding-left: 25px;"><?php echo $bottom_time ?>'</td>
			      <?php }
				  	} else { echo '<td width="50px"> ' . $bottom_time . "'" . '</td>' ;} ?>
					
					<td width="50px" align="left" background="<?php echo $wp.ICONS_FILEPATH . get_option('max_depth_icon_path')?>" style="background-repeat:no-repeat;padding-left: 25px;" ><?php echo $max_depth ?> <?php echo get_option('longitude')?></td>
